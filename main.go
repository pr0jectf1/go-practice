...
func placeOrder(productID string, quantity int, accountBalance float64) (bool, float64) {
    if stockAmount := amountInStock(productID); stockAmount < quantity {
        return false, accountBalance
    } else if orderTotal := calcPrice(productID, quantity); orderTotal > accountBalance {
        return false, accountBalance
    } else {
        return true, accountBalance - orderTotal
    }
}
